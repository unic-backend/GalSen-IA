## Done
- [x] **Tests de capacité absente rendus déterministes** — `conftest.py` (`module_absent`, `capacite_forcee`), `tests/test_capacite_absente.py`. 24 échecs et 6 erreurs qui ne venaient d'aucun changement de code : les tests mesuraient ce que le bac à sable avait installé. `faster-whisper` déclaré là où son poids le place (`requirements-audio.txt`), avec deux contre-tests qui empêchent la liste des déclarations d'exécution d'avaler `requirements-dev.txt`
