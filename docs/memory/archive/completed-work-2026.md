# GalSen IA — Completed Work (archive 2026, avant le 2026-08-09)

Entrées les plus anciennes, sorties de `docs/memory/completed-work.md` quand il a
dépassé 200 lignes (`.claude/rules/memory.md`). Rien n'est supprimé.
...
### 2026-08-06 (VOLET 02 ch. 08 — Phase 8.1 : les clés API ne subsistent plus en clair)
- **Trois fuites fermées.** Une clé API est un secret porteur : qui l'obtient agit à la place de son porteur. Elle était (1) conservée en clair dans `RBACManager._key_role_map` **et** dans `_valid_api_keys` du limiteur de taux, (2) journalisée partiellement — `key[:8]` en cas de rôle mal orthographié, huit caractères d'un secret divulgués —, (3) promenée dans `RBACContext.api_key` à chaque requête, donc susceptible d'apparaître dans une trace d'erreur ou d'audit. La clé servait en plus d'identifiant de seau au limiteur, ce qui la conduisait vers les journaux et les métriques.
- **Ce qui subsiste désormais** : le condensé SHA-256 seul. `hash_api_key()` et `key_fingerprint()` (12 caractères hexadécimaux) ajoutés ; `parse_api_key_mappings()` indexe par condensé ; `RBACContext` porte `key_fingerprint` au lieu de `api_key` ; `get_valid_keys()` → `get_valid_key_digests()` ; le limiteur reçoit des condensés et indexe ses seaux sur l'empreinte. La comparaison passe par `hmac.compare_digest` : la recherche par dictionnaire suffirait fonctionnellement mais n'est pas à temps constant.
- Vérifié en exécution : la clé en clair n'apparaît plus dans le mapping, l'authentification répond 200 avec la bonne clé et 401 avec une fausse.
- `tests/test_rbac.py` : +6 tests portant sur la propriété elle-même (aucune clé en clair en mémoire, aucune dans le contexte, aucune dans le journal en cas de rôle inconnu, condensé stable et non réversible, empreinte distinctive, limiteur sans clé). Suite complète : 905 tests passent, 5 ignorés.
- 2026-08-06 - **Phase 8.2 : posture de sécurité HTTP.** Trois trous constatés en exécution avant correction. (1) **CORS dangereux** : `allow_origins=["*"]` combiné à `allow_credentials=True` faisait renvoyer par Starlette l'origine de l'appelant *quelle qu'elle soit* — vérifié, `https://site-malveillant.example` recevait son propre nom en `Access-Control-Allow-Origin` avec `Allow-Credentials: true`, donc n'importe quel site pouvait appeler l'API avec les identifiants du visiteur. Défaut désormais : aucune origine, déclarées dans `GALSEN_CORS_ORIGINS`, méthodes et en-têtes restreints à ce que l'API utilise. (2) **Aucun en-tête de sécurité** sur aucune réponse : `SecurityHeadersMiddleware` pose `nosniff`, `X-Frame-Options: DENY`, `Referrer-Policy: no-referrer`, une CSP verrouillée (`default-src 'none'`) et `Cache-Control: no-store`. HSTS uniquement en HTTPS, `X-Forwarded-Proto` compris — en clair l'en-tête ne protège rien et casserait un accès local. (3) **Documentation publique** : `/docs`, `/redoc` et `/openapi.json` répondaient 200 sans clé. Règle retenue — *la documentation n'est publique qu'aussi longtemps que l'API l'est* : servie tant qu'aucune clé n'est configurée, fermée dès qu'il y en a une, `GALSEN_API_DOCS` tranchant explicitement. `src/api/security_headers.py` + `tests/test_api_security_headers.py` (26 tests). Suite complète : 931 tests passent, 5 ignorés.
- 2026-08-06 - **Phase 8.4 : révocation et rotation des clés API.** Une clé compromise restait valide jusqu'au redémarrage du serveur — or une clé fuite bien plus souvent qu'un disque ne se fait voler. Quatre routes réservées aux administrateurs : `GET /auth/keys` (inventaire par empreinte, rôle et état de révocation — aucune clé en clair), `POST /auth/keys/{empreinte}/revoke` (effet immédiat), `/restore` (lever une révocation posée par erreur), `POST /auth/keys/reload` (relit `GALSEN_API_KEYS` et resynchronise le limiteur, ce qui permet une rotation sans interruption : la nouvelle clé ouvre, l'ancienne ferme). Trois décisions : (1) on révoque **par empreinte**, l'opérateur n'a donc jamais à manipuler la clé ; (2) la réponse annonce `persistent: false` et rappelle de retirer la clé de l'environnement — la source durable reste le déploiement (ADR-004), et laisser croire l'incident clos serait pire que ne rien faire ; (3) une clé révoquée et une clé inventée reçoivent **le même message** de refus, car confirmer qu'une clé a existé renseignerait un attaquant. Le limiteur de taux suit les révocations, sans quoi une clé coupée conserverait son quota authentifié. Vérifié en exécution : la clé fuitée passe (200), est révoquée, puis reçoit 401. `tests/test_api_key_rotation.py` : 15 tests. Suite complète : 946 tests passent, 5 ignorés.
- 2026-08-06 - **Phase 8.3 : chiffrement au repos — chapitre 08 clos.** Un fichier SQLite copié depuis une sauvegarde livrait mémoires et connaissances en clair. `src/storage/encryption.py` chiffre le champ contenu des bases mémoire et connaissances (Fernet, clé dans `GALSEN_ENCRYPTION_KEY`, jamais dans le dépôt — ADR-004). Trois propriétés commandent la conception : (1) **activer le chiffrement ne casse pas une base existante** — une valeur sans la marque `galsen:v1:` traverse telle quelle, les anciennes lignes restent lisibles et les nouvelles écritures sont protégées ; (2) **une clé mal formée arrête l'opération** au lieu d'écrire en clair en donnant l'illusion d'une protection, et le message nomme la variable sans jamais contenir la clé ; (3) **une valeur illisible n'est jamais rendue** — mauvaise clé ou donnée corrompue lèvent, car rendre le texte chiffré comme s'il était le contenu ferait passer un échec pour une donnée. Vérifié en ouvrant le fichier en binaire : le contenu n'y figure pas, l'application le relit intact. `cryptography>=42` déclarée (la copie système 41.0.7 est cassée sur ce Python : `pyo3_runtime.PanicException`). `tests/test_storage_encryption.py` : 21 tests, dont le scénario de migration d'une base écrite avant activation. Suite complète : 967 tests passent, 5 ignorés.

### 2026-08-06 (VOLET 02 ch. 02 — Interface web, phase 2.1)
- **ADR-008 acceptée** : le client web démarre **sans étape de construction** — HTML, CSS et modules JavaScript servis tels quels par FastAPI sous `/ui`. Deux contraintes non techniques ont décidé : le projet est tenu par une personne, souvent depuis un téléphone, sur une connexion qui tombe (une chaîne d'outils à installer et déboguer avant le premier rendu est un coût payé en boucle) ; et la plateforme vise le Sénégal, où la bande passante ne se suppose pas (un premier chargement en mégaoctets décide de qui peut utiliser le produit). React + Vite est écarté *pour l'instant*, non sur le fond : c'est la bonne réponse pour une interface étendue, et le client d'API survivra à la migration puisqu'il ne dépend que de `fetch`.
- **Livré** : `src/web/static/` — `api-client.js` (seul module connaissant les routes), `dashboard.js` (panneaux santé, connecteurs, clés avec révocation), `dashboard.css` (thème clair/sombre par `prefers-color-scheme`, grille adaptative, aucune police téléchargée), `index.html`. Monté sur `/ui`, la racine `/` y redirige.
- **Défaut attrapé au navigateur, invisible en test HTTP** : la CSP posée en phase 8.2 (`default-src 'none'; sandbox`) rendait l'interface muette — `sandbox` interdit toute exécution de script. Une politique distincte s'applique désormais sous `/ui` (styles, scripts et connexions limités à la même origine, sans `unsafe-inline`), l'API gardant la sienne. Un client de test HTTP n'applique pas les CSP : seul un vrai navigateur pouvait le montrer.
- **Vérifié dans Chromium** : les trois panneaux affichent les données réelles, le CSS s'applique, aucune erreur de console une fois la clé saisie. Deux défauts de rendu corrigés au passage (titres de colonnes coupés en plein mot, pastilles d'état scindées sur deux lignes).
- **Non couvert, assumé** : la logique JavaScript n'a pas de tests unitaires. `tests/test_web_ui.py` (21 tests) couvre la couche de service, la CSP et des propriétés du code source (aucun `fetch` hors du client d'API, aucune écriture dans `innerHTML`, aucune ressource distante). L'ADR-008 fait de ce manque le signal qui justifiera un exécuteur de tests JS.
- Suite complète : 988 tests passent, 5 ignorés.

### 2026-08-06 (VOLET 02 ch. 10 — Montée en charge, phase 10.1 : VOLET 02 clos)
- **ADR-009 acceptée : la plateforme tourne en une seule instance, et le dit à l'exécution.** Le chapitre 10 réclame files d'attente, réplicas, cache distribué et auto-scaling. Construire cela aujourd'hui reviendrait à exploiter de l'infrastructure pour une plateforme sans fournisseur de modèle configuré, sans trafic et tenue par une personne — un coût payé chaque semaine pour une charge inexistante. La directive du chapitre ne demande pas l'état final, elle demande que la croissance **n'impose pas une refonte**. La question posée a donc été plus étroite et vérifiable : qu'est-ce qui casserait si une deuxième instance démarrait maintenant ?
- **Mesuré, pas supposé** : sept sous-systèmes portent de l'état, six dans le processus. Cinq sont bloquants avec le backend par défaut — révocations de clés, compteurs du limiteur de taux, fichiers téléversés, notifications, et l'état des moteurs, ce dernier cessant de l'être dès `GALSEN_STORAGE_BACKEND=sqlite` (ADR-005), seul point déjà réglé par configuration. Deux ne cassent rien (registres de connecteurs et de moteurs, reconstruits à l'identique depuis l'environnement).
- **Le trou dangereux a été démontré, pas déduit** : deux `RBACManager`, une clé révoquée sur le premier — le second l'authentifie toujours. Un opérateur qui réagit à une clé compromise croit l'incident clos alors que la clé ouvre encore les autres instances. L'échec est silencieux : l'API répond `revoked: true`.
- **La contrainte est exposée, pas documentée** : `src/api/scaling.py` inventorie l'état (portée, conséquence, caractère bloquant, réserve) ; `/health` porte une section `scaling` avec le verdict `multi_instance_ready` ; la réponse de révocation et `/auth/keys` annoncent `scope: "instance"`. Un opérateur lit la limite au moment où elle peut lui nuire, pas dans un fichier qu'il n'ouvrira peut-être jamais. L'inventaire se recalcule à chaque appel : figé à l'import, il mentirait dès qu'un déploiement change le backend.
- **L'ordre de réparation est fixé d'avance** (révocations → limiteur → fichiers → notifications), pour que la croissance soit incrémentale. Fichiers et notifications sont déjà derrière une interface de magasin : ce sera une substitution, pas une réécriture.
- `tests/test_scaling.py` : 20 tests, dont la démonstration de la révocation locale et la vérification que `/health`, non authentifiée, ne nomme aucun secret. Suite complète : **1008 tests passent**, 5 ignorés. **VOLET 02 terminé : 10 chapitres sur 10.**
- Implemented the Memory tool with French comments and ensured compliance with coding conventions (comments in French, clear method names, proper docstrings). Updated related tests pass.

### 2026-08-08 (Réconciliation des deux lignes de développement — 4 phases)
- **Deux branches avaient grandi en parallèle depuis `8ddd9c1` sans se connaître** : `main` (18 PR — connecteurs, posture de sécurité HTTP, chiffrement au repos, tableau de bord `/ui`, posture de montée en charge) et `feature/service-unit-tests` (un commit — services calendar/cloud/email, 5 magasins SQLite de service, SDK client Python, `POST /agri/advice`, serveur Ollama local). 12 conflits, résolus à la main. Trois arbitrages ont décidé : (1) `server.py` garde `lifespan` et écarte le `startup_event()` que la branche ramenait — il appelle trois méthodes qui n'existent pas, c'est le défaut qui empêchait l'API de démarrer ; (2) `agri_advice/tool.py` garde la version « statut », la branche levant `RuntimeError` là où l'indisponibilité est un statut et non une panne ; (3) **deux tableaux de bord** — `src/frontend/` (Jinja2, monté sur `/admin`) est retiré, `/ui` reste, l'ADR-008 est confirmée sous pression et non seulement en théorie.
- **Un défaut que les tests cachaient** : `POST /agri/advice` ne traduisait que les exceptions en 503. Avec l'outil qui retourne un statut, une plateforme sans modèle répondait **200 avec un conseil vide** — et `model_used` étant un `str` obligatoire, un `None` serait parti en 500. L'endpoint traduit désormais tout statut non-`ready` en 503 portant le détail de l'outil.
- **La convention d'imports enfreinte une seconde fois.** Les cinq services arrivés par la branche importaient `from storage...` en absolu, et `src/__init__.py` ajoutait `src/` à `sys.path` pour que ça résolve. Le hack masque la faute au lieu de la corriger : le même fichier devient importable sous deux noms, Python en fait deux modules, et `isinstance` échoue. Prouvé (`A is B` → False), puis corrigé — 10 modules et 6 fichiers de tests ramenés à `src.`, `src/__init__.py` vidé de toute logique. `tests/test_import_convention.py` parcourt désormais chaque module de `src/` et échoue au premier import nu ; le garde-fou a été vérifié en réintroduisant la faute.
- **Page « Conseil agricole » portée dans `/ui`** (`api.agri.conseil()`, section pleine largeur, réponse insérée en `textContent` avec ses retours à la ligne). Trois défauts de rendu trouvés **au navigateur seulement** : identifiants coupés en plein mot (`overflow-wrap: anywhere` → `break-word`), dernière colonne de tableau tronquée en silence (→ `table-layout: fixed`), en-têtes qui se chevauchaient (`nowrap` retiré). Vérifié dans Chromium à 1100 px et 390 px : aucune erreur console, aucun débordement horizontal.
- **Le rapport `scaling` (ADR-009) était devenu faux** : fichiers et notifications ont maintenant un magasin SQLite. `state_inventory()` dérive leur portée de la configuration au lieu de la déclarer — sous `sqlite`, il ne reste que les deux points qu'aucun backend ne règle : révocations de clés et compteurs de quota. C'est le mode d'échec que l'ADR-009 s'était prédit à elle-même, attrapé tôt.
- Retirés : `src/frontend/` et son montage `/admin`, `tests/test_dashboard_agri.py` (remplacé par `tests/test_web_agri.py`, 18 tests), les sondes temporaires `probe_agri.py` et `tests/probe_test.py` (cette dernière n'assertait rien), 5 bases `data/*.sqlite` et `.claude/settings.json.bak` sortis du suivi Git.
- ADR-008 et ADR-009 amendées. Suite complète : **1405 tests passent**, 5 ignorés.
