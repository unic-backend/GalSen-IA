...
- 2026-08-06 - Restent à trancher, repérés pendant la même passe : `src/model_engine/model_router.py` retourne `"Réponse simulée du modèle X"` et `src/model_engine/model_loader.py` retourne `status: "loaded"` sans rien charger. Ces deux classes sont construites par `ModelManagerImpl` mais **jamais appelées** — la génération réelle passe par `generate()` → `_call_provider`. Ce sont donc des mines, pas des mensonges actifs : les supprimer ou brancher le routeur sur le vrai chemin de génération est une décision d'architecture, laissée à l'utilisateur.
