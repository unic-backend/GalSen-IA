...
from .paths import default_sqlite_path, prepare_connection
...
class SQLiteUserStore(BaseRepository[User]):
    ...
    def _get_connection(self) -> sqlite3.Connection:
        """
        Ouvre une connexion SQLite préparée comme toutes celles de la plateforme.

        Les PRAGMA ne sont **pas** recopiés ici : `prepare_connection` est le
        seul endroit qui les pose (ADR-005). Ce module les écrivait à la main et
        en oubliait deux — `journal_mode=WAL` et `synchronous=NORMAL` — sans
        lesquels lecteurs et écrivain se bloquent mutuellement et une sauvegarde
        à chaud n'est pas possible. Un garde-fou du dépôt l'a signalé ; il avait
        raison, et c'est exactement ce qu'une seconde copie de règle produit.
        """
        if self.db_path == ":memory:":
            conn = sqlite3.connect(self._memory_uri, uri=True)
        else:
            conn = sqlite3.connect(self.db_path)
        return prepare_connection(conn)
